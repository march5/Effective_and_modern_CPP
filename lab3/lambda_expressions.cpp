...
int main(){
  ...
  std::sort(v.begin(), v.end(), [center](int a, int b){
	  if (abs(50 - a) <= abs(50 - b)) return true;
	  else return false;
	  });            // 1
  print(v, "center 50:");
  ...
  auto positiveEvenFirst= [](int a, int b){
	  if (a == 0 && b > 0) return false;
		  ...
		  else if (a < b) return true; else return false;
	  }
  };                  // 2
  ...
  int start = 5, step =2;
  auto arithmeticGenerator =  [start, &step, z = start]() mutable{
	  z += step;
	  return z - step;
  } ;                   // [ 4 ]
  ...
  auto l1_norm = [](items) mutable{
	  return accumulate(items.begin(), items().end, 0, [](int total, int i) {return total + i; });
	};;		// [ 5 ]
